/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  DRDD DIPE CITY — AssetManager.js  (v16.13.0)                     ║
    ...
    bootstrapLoader().then(function (boot) {
      ...
      keys.forEach(function (key) {
        ...
        loadFile(spec.file).then(function (gltf) {
          try {
            // first consumer takes the parsed scene itself; later consumers of
            // the same file take an independent clone so per-key rotX/height
            // normalisation can never fight over one scene graph
            var root;
            if (!gltf.__claimedBy) { gltf.__claimedBy = key; root = gltf.scene; }
            else root = skeletonClone ? skeletonClone(gltf.scene) : gltf.scene.clone(true);
            tuneMaterials(root, key);                // V16.4.0 — before caching
            ...
            okC++; diag('log', 'GLB ready: ' + key + ' ← ' + spec.file +
              (gltf.__claimedBy === key ? '' : ' (shared download)') +
...
/* ═══════════════════════════════════════════════════════════════════════════
   MUSIC (V16.9.0)
   Streams looping background tracks from assets/music/. Missing files are
   silently skipped — the game never depends on a track existing, so tracks
   can be added one at a time as they are finished.
   ...
   Browsers refuse to start audio before the person has touched the page
   (autoplay policy), so the first track is queued and started by a one-time
   unlock listener on the first tap / click / key.
   ═══════════════════════════════════════════════════════════════════════════ */
window.Music = (function () {
  ...
  var pool = {}, missing = {}, cur = null, curKey = null;
  var pending = null, muted = false, VOL = 0.55, wanted = null;
  ...
  function get(key) {
    ...
    if (!path) { missing[key] = true; return null; }
    try {
      var a = new Audio(path);
      a.loop = true; a.preload = 'auto'; a.volume = 0;
      // V16.12.0: a load error no longer kills the track forever. During the
      // first minutes of a session the model download can saturate the
      // connection and stall an .mp3 — permanently blacklisting it meant a
      // track that merely lost that race stayed silent for the whole session.
      // Now the dead element is dropped and the NEXT request for the same key
      // (re-entering the level, starting another match) simply tries again,
      // up to 4 total attempts before the key is treated as genuinely absent.
      a.onerror = function () {
        attempts[key] = (attempts[key] || 0) + 1;
        delete pool[key];
        if (attempts[key] >= 4) missing[key] = true;
        if (curKey === key) { cur = null; curKey = null; }
      };
      pool[key] = a; return a;
    } catch (e) { missing[key] = true; return null; }
  }
  // simple linear fade on a timer; each audio carries its own so a crossfade
  // (old fading down, new fading up) just works
  function fade(a, to, ms, then) {
    if (!a) { if (then) then(); return; }
    if (a.__fade) clearInterval(a.__fade);
    var from = a.volume, steps = Math.max(1, Math.round(ms / 40)), i = 0;
    a.__fade = setInterval(function () {
      i++; a.volume = Math.max(0, Math.min(1, from + (to - from) * (i / steps)));
      if (i >= steps) { clearInterval(a.__fade); a.__fade = null; if (then) then(); }
    }, 40);
  }
  /** Silence EVERYTHING immediately.
   *  V16.13.0: this used to fade only `cur`. Any other element still running —
   *  a track mid-crossfade, one resumed by the autoplay unlock, one left over
   *  from a retry — kept playing, which is how tracks ended up stacked on top
   *  of each other after a few level changes. Now every element in the pool is
   *  paused and rewound, so there is never more than one source of sound. */
  function hardStop() {
    for (var k in pool) {
      var a = pool[k];
      if (!a) continue;
      try { if (a.__fade) { clearInterval(a.__fade); a.__fade = null; } } catch (e) {}
      try { a.pause(); a.currentTime = 0; a.volume = 0; } catch (e) {}
    }
    cur = null; curKey = null; pending = null; wanted = null;
  }
  function stop(ms) {
    if (cur && ms) {                       // optional short fade for the current one
      var old = cur;
      fade(old, 0, ms, function () { try { old.pause(); old.currentTime = 0; } catch (e) {} });
      cur = null; curKey = null; pending = null; wanted = null;
      return;
    }
    hardStop();
  }
  /** Begin buffering a track. Resolves once it can play through without
   *  stalling, or on error/timeout — never rejects, so a caller can always
   *  just await it and carry on. V16.13.0. */
  function preload(key, timeoutMs) {
    return new Promise(function (res) {
      var a = get(key);
      if (!a) { res(false); return; }
      if (a.readyState >= 3) { res(true); return; }        // HAVE_FUTURE_DATA+
      var done = false;
      function finish(ok) {
        if (done) return; done = true;
        a.removeEventListener('canplaythrough', okFn);
        a.removeEventListener('error', errFn);
        res(ok);
      }
      function okFn() { finish(true); }
      function errFn() { finish(false); }
      a.addEventListener('canplaythrough', okFn);
      a.addEventListener('error', errFn);
      try { a.load(); } catch (e) {}
      setTimeout(function () { finish(a.readyState >= 3); }, timeoutMs == null ? 6000 : timeoutMs);
    });
  }

  /** Start a track NOW (assumes it is buffered, or accepts a slow start). */
  function play(key) {
    if (missing[key] || !pathFor(key)) { hardStop(); return; }
    if (curKey === key && cur && !cur.paused) return;      // already playing it
    var next = get(key);
    if (!next) { hardStop(); return; }
    // V16.13.0: stop everything BEFORE starting the new track. The old code
    // fade-out overlapped the new track by 600ms by design, and any element
    // that was not `cur` was never touched at all.
    hardStop();
    cur = next; curKey = key; wanted = key;
    if (muted) return;                                     // remembered, not started
    try { next.currentTime = 0; } catch (e) {}
    next.volume = 0;
    var p = next.play();
    if (p && p.catch) p.catch(function () { pending = key; });  // blocked → wait for unlock
    fade(next, VOL, 400);
  }

  /** Buffer first, then start — so a level's music begins WITH the level
   *  instead of arriving several seconds in. `wanted` guards against a fast
   *  level change: if the player has moved on by the time this resolves, the
   *  stale track is dropped instead of starting over the new one. */
  function playWhenReady(key, timeoutMs) {
    if (missing[key] || !pathFor(key)) { hardStop(); return Promise.resolve(false); }
    hardStop();
    wanted = key;
    return preload(key, timeoutMs).then(function (ok) {
      if (wanted !== key) return false;                    // level changed — abandon
      play(key);
      return ok;
    });
  }
  // V16.12.0: this listener used to fire ONCE, on the first tap — which on a
  // phone is a menu tap made before any track has even been requested, so it
  // spent its one shot on nothing. It now stays alive for the whole session:
  // every tap or key is a fresh chance to start a track that the browser's
  // autoplay policy (or a lost bandwidth race) previously blocked. It is a
  // few comparisons per tap — free.
  function unlock() {
    if (muted) return;
    if (pending) { var k = pending; pending = null; if (wanted === k) play(k); }
    else if (cur && cur.paused && curKey === wanted) {
      try { var p = cur.play(); if (p && p.catch) p.catch(function () {}); fade(cur, VOL, 400); } catch (e) {}
    }
  }
  ['pointerdown', 'touchstart', 'keydown'].forEach(function (ev) {
    try { window.addEventListener(ev, unlock, { capture: true }); } catch (e) {}
  });
  ...
  return {
    ...
    /** Buffer a level's track WITHOUT starting it — used by the level-load
     *  overlay so the music is ready the instant the level appears. */
    prepareLevel: function (which, timeoutMs) {
      var k = LEVEL_KEY[which] || ('level' + which);
      hardStop(); wanted = k;
      if (missing[k] || !pathFor(k)) return Promise.resolve(false);
      return preload(k, timeoutMs);
    },
    /** Start whatever prepareLevel() staged, if the level hasn't changed. */
    startPrepared: function () { if (wanted) play(wanted); },
    /** Called at Dipe Fighter match start with the chosen character's id. */
    fighter: function (id) { return playWhenReady('fight_' + id); },
    play: play, stop: stop, hardStop: hardStop, preload: preload,
    setVolume: function (v) { VOL = Math.max(0, Math.min(1, v)); if (cur && !muted) fade(cur, VOL, 200); },
    toggleMute: function () {
      muted = !muted;
      try { localStorage.setItem('drdd_music_muted', muted ? '1' : '0'); } catch (e) {}
      if (muted) { if (cur) fade(cur, 0, 250); }
      else if (cur) { try { cur.play(); } catch (e) {} fade(cur, VOL, 400); }
      return muted;
    },
