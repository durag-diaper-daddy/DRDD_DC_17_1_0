...
window.Music = (function () {
  ...
  return {
    ...
    /** V16.14.0 — buffer tracks in the background, ONE AT A TIME, starting as
     *  soon as the page loads. Sequential on purpose: firing 20+ parallel
     ...
     *  first, since those are what a player reaches soonest; the per-fighter
     *  themes follow. A track already buffering or played is skipped, and a
     *  failure just moves to the next one. */
    prefetchAll: function () {
      ...
      (function next() {
        ...
        var key = order[i++];
        if (pool[key] || missing[key]) { next(); return; }   // already handled
        preload(key, 20000).then(next, next);
      })();
    },
