/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  DRDD DIPE CITY — AssetManager.js  (v16.11.0)                     ║
...
window.Music = (function () {
  ...
  var pending = null, unlocked = false, muted = false, VOL = 0.55;
  ...
  function get(key) {
    ...
    try {
      ...
      a.onerror = function () { missing[key] = true;      // 404 → skip forever,
        if (curKey === key) { cur = null; curKey = null; } };   // never retry-spam
      pool[key] = a; return a;
  ...
  function unlock() {
    ...
    if (pending && !muted) { var k = pending; pending = null; play(k); }
    else if (cur && cur.paused && !muted) { try { cur.play(); fade(cur, VOL, 700); } catch (e) {} }
  }
  ['pointerdown', 'touchstart', 'keydown'].forEach(function (ev) {
    try { window.addEventListener(ev, unlock, { once: true, capture: true }); } catch (e) {}
  });
