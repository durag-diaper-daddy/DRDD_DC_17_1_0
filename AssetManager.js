/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  DRDD DIPE CITY — AssetManager.js  (v16.12.0)                     ║
...
window.Music = (function () {
  ...
  var pending = null, muted = false, VOL = 0.55;
  ...
  function stop(ms) {
    if (cur) { var old = cur; fade(old, 0, ms == null ? 500 : ms, function () { try { old.pause(); } catch (e) {} }); }
    cur = null; curKey = null; pending = null;
  }
  function play(key) {
    if (missing[key] || !pathFor(key)) { stop(); return; }
    if (curKey === key && cur && !cur.paused) return;      // already playing it
    var next = get(key); if (!next) { stop(); return; }
    if (cur && cur !== next) { var old = cur; fade(old, 0, 600, function () { try { old.pause(); } catch (e) {} }); }
    cur = next; curKey = key; pending = null;
    if (muted) return;                                     // remembered, not started
    try { next.currentTime = next.currentTime; } catch (e) {}
    var p = next.play();
  ...
  function unlock() {
    if (muted) return;
    if (pending) { var k = pending; pending = null; play(k); }
    else if (cur && cur.paused) { try { var p = cur.play(); if (p && p.catch) p.catch(function () {}); fade(cur, VOL, 700); } catch (e) {} }
  }
  ...
  return {
    ...
    level: function (which) { play(LEVEL_KEY[which] || ('level' + which)); },
    /** Called at Dipe Fighter match start with the chosen character's id. */
    fighter: function (id) { play('fight_' + id); },
    play: play, stop: stop,
